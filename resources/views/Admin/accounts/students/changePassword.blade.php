...
@section('main-content')
    ...
    <div class="max-w-5xl bg-white rounded-lg shadow p-6 my-10 ml-70">
        ...
        <!-- Title -->
        <h2 class="text-xl font-semibold mb-6">Create/Update Account Password</h2>

        <!-- Search Section -->
        <form action="{{ route('student.changePassword') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-2 mb-8">
                <div>
                    <label class="block text-sm font-medium mb-1">User ID</label>
                    <input type="text" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring" name="id"
                        value="{{ old('id') }}" placeholder="1234">
                </div>
                ...
                <div class="flex items-end">
                    <button type="submit" class="w-20 bg-green-600 text-white py-2 rounded hover:bg-green-700">
                        Search
                    </button>
                </div>
            </div>
        ...
        <div class="flex flex-col md:flex-row gap-6 items-start mt-[70px]">
            ...
            @if (count($student) > 0)
                ...
                <form action="{{ route('student.updatePassword', encrypt($student->id)) }}" method="POST">
                    @csrf
            ...
            @else
                ...
                <div class="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
                    <div>
                        <label class="block text-sm font-medium mb-1">User ID</label>
