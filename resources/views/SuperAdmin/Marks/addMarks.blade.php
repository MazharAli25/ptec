...
@section('main-content')
    ...
    <div class="flex-1 p-8 ml-[19vw]">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Add Total Marks</h1>
            <p class="text-gray-600 mt-2">Fill in the details to add marks</p>
        </div>


        <!-- Form Section -->
        <div class="bg-white rounded-lg form-shadow p-6 mb-8 flex justify-center">
            <form class="flex flex-col w-[100%]" method="POST" action="{{ route('marks.store') }}">
                @csrf
                <div class="space-y-4">
                    <h2 class="text-lg font-semibold text-gray-700 border-b pb-2">Marks Information</h2>
                    <div class="flex justify-center gap-4">

                        <div>
                            <label for="subect_id" class="block text-sm font-medium text-gray-700 mb-1 w-[400px]">
                                Select Subject
                            </label>
                            <select type="text" id="subject_id" name="subject_id" placeholder="Enter Marks Here"
                        ...
                        <div>
                            <label for="marks" class="block text-sm font-medium text-gray-700 mb-1 w-[400px]">
                                Add Total Marks
                            </label>
                            <input type="text" id="marks" name="marks" placeholder="Enter Marks Here"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </input>
                            @error('marks')
                                <p class="text-red-500 text-[14px] font-semibold">{{ $message }}</p>
