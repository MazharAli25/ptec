...
@section('main-content')
    ...
    <div class="flex-1 p-8 ml-[19vw]">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Add Result Status</h1>
            <p class="text-gray-600 mt-2">Fill in the details to add a result status</p>
        </div>


        <!-- Form Section -->
        <div class="bg-white rounded-lg form-shadow p-6 mb-8 flex justify-center">
            <form class="flex flex-col w-[70%]" method="POST" action="{{ route('result.store') }}">
                @csrf
                <div class="space-y-4">
                    <h2 class="text-lg font-semibold text-gray-700 border-b pb-2">Result Status Information</h2>
                    <div class="flex justify-center">
                        <div>
                            <label for="courseName" class="block text-sm font-medium text-gray-700 mb-1 w-[400px]">
                                Add Result Status
                            </label>
                            <input type="text" id="status" name="status" placeholder="Enter Result Status"
        ...
        <div class="bg-white rounded-lg form-shadow p-6">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-semibold text-gray-800">Result Status List</h2>
                        ...
                        @foreach ($results as $result)
                            <tr>
                                ...
                                <td class="px-6 py-3 text-left text-[14px] font-medium text-gray-600 tracking-wider">
                                    {{ $result['status'] }}</td>
                                <td
                                    class="px-6 py-3 w-[35%] text-center text-xs font-medium text-gray-600 tracking-wider">
                                  <a href="#"
